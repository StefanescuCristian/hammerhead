...
static inline cputime64_t get_cpu_idle_time_eX(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(od_cpu_dbs_info, j);
		dbs_info->prev_cpu_idle = get_cpu_idle_time_eX(j,
...
static unsigned int get_cpu_current_load(unsigned int j, unsigned int *record)
{
	...
	cur_idle_time = get_cpu_idle_time_eX(j, &cur_wall_time);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time_eX(j,
...
static int cpufreq_gov_dbs_up_task(void *data)
{
	...
	while (1) {
		...
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time_eX(cpu, &this_dbs_info->prev_cpu_wall);
